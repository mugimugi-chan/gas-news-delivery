...
1. GASエディタの左メニューから **「トリガー（時計マーク）」** を選択します。
2. 右下の **「+ トリガーを追加」** をクリックします。
3. 以下の通り設定して保存します。
   - **実行する関数を選択**: 目的の関数を選択してください
     - `sendDailyNewsToAll` : メールとLINEの両方に一括送信
     - `sendDailyNewsToGmail` : メール（Gmail）のみに送信
     - `sendDailyNewsToLine` : LINE（Flex Message）のみに送信
   - **実行するデプロイを選択**: `Head`
   - **イベントのソースを選択**: `时间主導型`
   - **時間ベースのトリガーのタイプを選択**: `日付ベースのタイマー`
   - **時刻を選択**: `午前 8時～9時`（またはお好みの時間帯）

> 💡 **NOTE (GASの実行時刻について)**  
> GAS標準の時間主導型トリガーは、「午前9時〜10時」のように1時間の枠の中でランダムに実行される仕様になっています。分単位でジャストな時間に実行したい場合は、毎朝早朝に特定時刻のトリガーを動的生成する構成も可能です(本リポジトリでは対応しませんので，その機能が欲しい方は自作してください)。
